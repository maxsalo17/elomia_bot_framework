...
class BootBot extends EventEmitter {
  ...
  constructor(options) {
    super();
    if (!options || (options && (!options.accessToken || !options.apiEndpoint))) {
      throw new Error('You need to specify an accessToken and apiEndpoint');
    }
  ...
  start(port) {
    ...
    this.server = this.app.listen(this.app.get('port'), () => {
      ...
      console.log('ElomiaAppBot running on port', portNum);
      console.log(`Elomia Webhook running on localhost:${portNum}${this.webhook}`);
    });
  }
   ...
   * @param {SendMessageOptions} options
   */
  sendTextWithObject(recipientId, text, object, quickReplies, options) {
    const message = {
      type: 'object',
      text: text,
      object: object
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    return fetch(`${this.apiEndpoint}${endpoint}?access_token=${this.accessToken}`, {
      method,
      headers: {
      ...
      .then(res => {
        if (res.error) {
          console.log('Elomia Error received. For more information about error codes, see: https://goo.gl/d76uvB');
  ...
  /**
   * Thread API has been replaced by the Elomia Profile API.
  ...
  getUserProfile(userId) {
    const url = `${this.apiEndpoint}/users/${userId}?access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
    ...
    return buttons && buttons.map((button) => {
      if (typeof button === 'string') {
        return {
          type: 'postback',
          title: button,
          payload: 'APPBOT_BUTTON_' + normalizeString(button)
        };
    ...
    return quickReplies && quickReplies.map((reply) => {
      if (typeof reply === 'string') {
        return {
          content_type: 'text',
          title: reply,
          payload: 'APPBOT_QR_' + normalizeString(reply)
        };
      } else if (reply && reply.title) {
        return Object.assign({
          content_type: 'text',
          payload: 'APPBOT_QR_' + normalizeString(reply.title)
        }, reply);
      }
      return reply;
    });
